(function($) {
    ...
    function support3d() {
        ...
        var el = document.createElement('div'), has3d, transform = supportCSS3('transform');
    ...
    var PLUGIN = 'al_Slider';

    var al_Slider = function(el, options) {
        ...
        function init() {
            ...
            if (settings.touch) {
                // if the image img tag set attribute draggable false
            ...
            if (settings.arrows) {
                ...
                $arrows.on('click.' + PLUGIN, function(e) {
                    e.preventDefault();
                    if (this.getAttribute('data-al_Slider-dir') === 'next') {
                        show(current + 1);
                    } else {
                        show(current - 1);
                    }
        ...
        function _controls() {
            ...
            for (var i = 0; slideLen > i; i++) {
                ...
                links.push('<a class="slider-nav__control ' + act + '" data-al_Slider-control="' + i + '"></a>');
            }
            ...
            $controlPanel.on('click.' + PLUGIN, '.slider-nav__control', function(e) {
                ...
                if ($(this).hasClass('is-active'))
                    return;
                show(parseInt(this.getAttribute('data-al_Slider-control'), 10));
            });
        }
        ...
        function _move(value, hasAnimate) {
            ...
            if (transitionProperty && transformProperty) {
                ...
                (hasAnimate) ? sliderStyle[transitionProperty] = transformProperty + ' ' + settings.speed + 'ms ' + settings.cssEase : sliderStyle[transitionProperty] = "none";

                (has3d) ? sliderStyle[transformProperty] = 'translate3d(' + value + 'px, 0, 0)' : sliderStyle[transformProperty] = 'translateX(' + value + 'px)';
        ...
        function destroy() {
            ...
            if (settings.touch) {
                $slider.removeClass('has-touch').off(Touch.event().start + '.' + PLUGIN).off(Touch.event().move + '.' + PLUGIN).off(Touch.event().end + '.' + PLUGIN).off(Touch.event().leave + '.' + PLUGIN);
                touchFlag = false;
            }
            if (settings.responsive) {
    ...
    $.fn[PLUGIN] = function(opt) {
        ...
        this.each(function() {
            ...
            if (!slider && /(destroy|\d+)/.test(opt))
                return;
            if (!slider) {
                slider = new al_Slider($this,options);
                $this.data('al_Slider', slider);
                slider.init();
            }
            if (typeof opt === 'string' || typeof opt === 'number' && opt !== 'init') {
                if (typeof opt === 'number') {
                    ...
                    if (slider[opt]) {
                        ...
                        throw new Error('Error:: al_Slider has no method: ' + opt);
                    }
                }
            }
            return _this;
